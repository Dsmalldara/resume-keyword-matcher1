...
import { FileText, Users, Trash2, Send, User, Briefcase, Clock, ChevronRight, TrendingUp, Upload, Trash } from 'lucide-react';
...
const getActivityColor = (type: string) => {
  const colorMap: Record<string, { bg: string; icon: string; bgDark: string; iconDark: string }> = {
    RESUME_UPLOADED: {
      bg: "bg-blue-100",
      icon: "text-blue-600",
...
const shouldUseEnhancedUI = (message: string): boolean => {
  const keywords = ['analyz', 'delet', 'upload'];
  const lowerMessage = message.toLowerCase();
    ...
    >
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0 relative z-10">
        ...
        >
          ...
          {isHovered && (
            <div className={`absolute inset-0 rounded-xl ${colors.bg} animate-ping opacity-75`} />
          )}
        </div>
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors duration-200 group cursor-pointer">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        >
          <div className={`${colors.icon} ${colors.iconDark}`}>
            {icon}
          </div>
        </div>
